...
# -*- coding: utf-8 -*-
"""
Traductor XLIFF Simple - Inglés a Gallego
...
class TraductorXLIFFGallego:
    ...
    def __init__(self, archivo_entrada: str, archivo_salida: str = None):
        ...
        if archivo_salida is None:
            ...
            self.archivo_salida = str(
                entrada_path.parent / f"{entrada_path.stem}_gl{entrada_path.suffix}"
            )
        else:
            self.archivo_salida = archivo_salida

        self.traductor = GoogleTranslator(source='en', target='gl')
        self.checkpoint_file = "checkpoint_gallego.json"
    ...
    def traducir(self, texto: str, reintentos: int = 3) -> str:
        """Traduce un texto del inglés al gallego"""
    ...
    def procesar(self):
        ...
        print(f"🔄 Iniciando traducción al gallego...\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Traductor XLIFF simple - Inglés a Gallego (sin API key)'
    )
    parser.add_argument(
        'archivo',
    ...
    parser.add_argument(
        '-o', '--output',
        help='Archivo de salida (por defecto: archivo_gl.xliff)'
    )
    ...
    try:
        traductor = TraductorXLIFFGallego(args.archivo, args.output)
        traductor.procesar()
