...
# -*- coding: utf-8 -*-
"""
Traductor XLIFF Simple - Uso inmediato sin API keys
...
class TraductorXLIFFSimple:
    ...
    def __init__(self, archivo_entrada: str, archivo_salida: str = None):
        ...
        if archivo_salida is None:
            ...
            self.archivo_salida = str(
                entrada_path.parent / f"{entrada_path.stem}_ca{entrada_path.suffix}"
            )
        else:
            self.archivo_salida = archivo_salida
        
        self.traductor = GoogleTranslator(source='en', target='ca')
        self.checkpoint_file = "checkpoint_simple.json"
    ...
    def procesar(self):
        ...
        print(f"🔄 Iniciando traducción al catalán...\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Traductor XLIFF simple - Inglés a Catalán (sin API key)'
    )
    parser.add_argument(
        'archivo',
    ...
    parser.add_argument(
        '-o', '--output',
        help='Archivo de salida (por defecto: archivo_ca.xliff)'
    )
    ...
    try:
        traductor = TraductorXLIFFSimple(args.archivo, args.output)
        traductor.procesar()
